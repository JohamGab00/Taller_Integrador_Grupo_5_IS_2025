...
╚══════╝ ╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═╝    ╚═╝  ╚═╝╚═╝     ╚═╝  ╚═╝╚══════╝
      ...
      ╚══██╔══╝██╔══██╗██╔══██╗██╔════╝██║ ██╔╝██╔════╝██╔══██╗
         ...
         ╚═╝   ╚═╝  ╚═╝╚═╝  ╚═╝ ╚═════╝╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝
                                ...
                                GRUPO 5  --  TEC
            Natalia Hernández - Joham Mora - Josué Marín
          https://github.com/JohamGab00/Taller_Integrador_Grupo_5_IS_2025
...
// ===================== ZONA DE ALERTA: Definición del pin =====================
// Pin para botón de alerta con pull-up interno
#define ALERT_BUTTON_PIN 32
// =============================================================================
...
void setup() {
    ...
    // ===================== ZONA DE ALERTA: Configurción del pin del botón =====================
    // Configurar como pull-up interno
    pinMode(ALERT_BUTTON_PIN, INPUT_PULLUP);
    logger.log(logging::LoggerLevel::LOGGER_LEVEL_INFO,
               "Main", "Botón de alerta configurado en GPIO32 con pull-up interno");
    // =============================================================================
    ...
    WiFi.mode(WIFI_OFF);
    logger.log(logging::LoggerLevel::LOGGER_LEVEL_DEBUG,
               "Main", "WiFi controller stopped");
    if (bluetoothActive) {
        if (Config.bluetooth.useBLE) BLE_Utils::setup();
        else {
#ifdef HAS_BT_CLASSIC
...
void loop() {
    // ===================== ZONA DE ALERTA: Variables de estado =====================
    static bool alertSent = false;
    static uint32_t alertUntil = 0;
    // =============================================================================

    // ===================== ZONA DE ALERTA: Lectura del botón y envío de paquete =====================
    bool btn = digitalRead(ALERT_BUTTON_PIN);
    if (btn == LOW && !alertSent) {
        alertSent = true;
        // Construir y enviar paquete APRS de emergencia
        String em = String(currentBeacon->callsign)
            + ">APZTRK:!EMERGENCY ALERT!";
        LoRa_Utils::sendNewPacket(em);
        // Mostrar alerta en pantalla y fijar duración
        alertUntil = millis() + 1000;
    } else if (btn == HIGH && alertSent) {
        // Al soltar el botón, permitir nuevo envío
        alertSent = false;
    }
    // =============================================================================

    // ===================== ZONA DE ALERTA: Mostrar alerta en pantalla =====================
    if (millis() < alertUntil) {
        displayShow("", "", "¡ALERTA APRS!",
                    "Enviado a red APRS", "", "");
        // Salir del loop para que no se ejecute la lógica normal mientras dure la alerta
        return;
    }
    // =============================================================================

    // ===================== Lógica normal GPS / menús / refresco =====================
